...
describe('Fighter', () => {
    ...
    it('should correctly instantiate a fighter with the expected default range of values', () => {
        ...
        const validInitiativeRollBonus = fighterInstance.initiativeRollBonus() > 0 && fighterInstance.initiativeRollBonus() <= 4;
        expect(validInitiativeRollBonus).toBeTruthy();

        const validAttackRollBonus = fighterInstance.attackRollBonus() > 0 && fighterInstance.attackRollBonus() <= 6;
        expect(validAttackRollBonus).toBeTruthy();

        expect(fighterInstance.attackDamageRoll()).toStrictEqual({ 4: 3 });
        expect(fighterInstance.HP()).toBe(100);

        const validDamageReduction = fighterInstance.damageReduction() >= 2 && fighterInstance.damageReduction() <= 6;
        expect(validDamageReduction).toBeTruthy();

        const validArmorClass = fighterInstance.AC() > 10 && fighterInstance.AC() <= 17;
    ...
    it('should correctly instantiate a fighter with given values', () => {
        ...
        expect(fighterInstance.attackDamageRoll()).toStrictEqual({4:3, 6: 2});
